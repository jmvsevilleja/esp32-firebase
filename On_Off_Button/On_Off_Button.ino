...
void setup(void)
{
  ...
  */
  ...
  tft.setCursor (0, 40);
  tft.print("Connected with Firebase...");
  ...
  if(Firebase.getBool(fbdo,"/SwitchOn/data"))
  {
    ...
    tft.setCursor (0, 50);
    tft.print("Error connecting Firebase:");
    tft.setCursor (0, 60);
    ...
    if (SwitchOn)
    {
      if ((x > REDBUTTON_X) && (x < (REDBUTTON_X + REDBUTTON_W))) {
        if ((y > REDBUTTON_Y) && (y <= (REDBUTTON_Y + REDBUTTON_H))) {
          ...
          {
            //Success
             Serial.println("Set false data success");
             redBtn();
          }else{
            //Failed?, get the error reason from fbdo
    ...
    else //Record is off (SwitchOn == false)
    {
      if ((x > GREENBUTTON_X) && (x < (GREENBUTTON_X + GREENBUTTON_W))) {
        if ((y > GREENBUTTON_Y) && (y <= (GREENBUTTON_Y + GREENBUTTON_H))) {
          ...
          {
            //Success
             Serial.println("Set true data success");
             greenBtn();
          }else{
            //Failed?, get the error reason from fbdo
